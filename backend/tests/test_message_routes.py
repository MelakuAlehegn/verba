...
def test_list_messages_returns_history(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda *a, **k: [_message("user", "hi"), _message("assistant", "(stub) You said: hi")],
    )
...
def test_post_message_returns_user_and_assistant(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda db, chat, payload: (
            _message("user", payload.content),
            _message("assistant", f"(stub) You said: {payload.content}"),
        ),
    )
    ...
    assert "hello" in body["assistant_message"]["content"]
